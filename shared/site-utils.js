...
export async function loadPageContent(contentFiles) {
  if (typeof window !== 'undefined' && window.__CONTENT__ && Object.keys(window.__CONTENT__).length > 0) {
...
export async function loadAndRenderPage({ contentFiles, renderers, onAfterRender }) {
...
export function showFormMessage(el, type, text) {
  if (!el) {
    return;
  }
...
export async function submitContactForm(event, options = {}) {
...
// Browser-compat: expose on window for any remaining non-module consumers
if (typeof window !== 'undefined') {
  window.VBTUtils = Object.assign(window.VBTUtils || {}, {
